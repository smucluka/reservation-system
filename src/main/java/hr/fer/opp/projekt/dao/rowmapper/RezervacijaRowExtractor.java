...
public class RezervacijaRowExtractor implements ResultSetExtractor<List<Rezervacija>> {

	@Override
	public List<Rezervacija> extractData(ResultSet rs) throws SQLException, DataAccessException {
		Map<Integer, Rezervacija> map = new HashMap<Integer, Rezervacija>();
		Rezervacija rezervacija = null;
		while (rs.next()) {
			Integer id = rs.getInt("ID_REZERVACIJA");
			rezervacija = map.get(id);
			if (rezervacija == null) {
	            ...
	            map.put(id, rezervacija);
			}
			rezervacija.add(rs.getInt("ID_USLUGE"), rs.getString("NAZ_USLUGE"));
		}
		return new ArrayList<Rezervacija>(map.values());
